    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            )
            {
                ...
                const float blockOffset = ComputeBlockOffset(input.positionWS);
                const float clipValue = ComputeRevealClip(input.positionWS, blockOffset);
                clip(clipValue);
                ...
                if (_BlockOffsetSteps <= 0.0f && _RevealFeather > 0.0001f)
                {
                    ...
                    const half fade = saturate(clipValue / safeFeather);
