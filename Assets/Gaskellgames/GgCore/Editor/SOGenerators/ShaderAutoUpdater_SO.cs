...
namespace Gaskellgames.EditorOnly
{
    ...
    public class ShaderAutoUpdater_SO : GgScriptableObject
    {
        ...
        private void UpdateMaterials(Pipeline pipeline)
        {
            switch (pipeline)
            {
                case Pipeline.BIRP:
                    if (!BIRP)
                    {
                        Log("Error: Missing shader reference: ShaderAutoUpdater failed to update shaders for the current pipeline.", LogType.Error);
                        break;
                    }
                    for (int i = materials.Count - 1; i >= 0; i--)
                    {
                        ...
                        Log($"Material [{materials[i].name}] updated to use shader [{BIRP}].");
                    }
                    break;
                ...
                case Pipeline.URP:
                    ...
                    for (int i = materials.Count - 1; i >= 0; i--)
                    {
                        ...
                        Log($"Material [{materials[i].name}] updated to use shader [{URP}].");
                    }
                    break;
                ...
                case Pipeline.HDRP:
                    ...
                    for (int i = materials.Count - 1; i >= 0; i--)
                    {
                        ...
                        Log($"Material [{materials[i].name}] updated to use shader [{HDRP}].");
                    }
                    break;
                ...
                default:
                    Log("Error: Other or Unknown pipeline detected. ShaderAutoUpdater failed to update shaders for the current pipeline.", LogType.Error);
                    break;
            }
        }
        ...
        private Pipeline GetTargetRenderPipeline()
        {
            ...
            {
                Log($"Using override render pipeline: {pipeline}");
                return pipeline;
            }
            
            pipeline = GetDefaultRenderPipeline();
            Log($"Using default render pipeline: {pipeline}");
            return pipeline;
        }
